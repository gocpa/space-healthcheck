...
- **`Git.php` читает `.git/` файлами, не шеллом.** `HEAD` → `refs/heads/<branch>` → zlib-распакованный объект коммита для даты. Так работает там, где `exec()` запрещён, а бинарника git нет в контейнере. Конструктор бросает исключение, если `.git/` отсутствует; контроллер это ловит и кладёт `git.exception` в ответ.
